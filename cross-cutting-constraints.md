...
| 风险 | 说明 |
|------|------|
| 只验登录不验归属 | 拦截器/Filter 不能代替「按 ID 读写」 |
| 列表透传 `userId` | Service 用 `request.getUserId()` 且 API 未强制 session → 可查他人数据 |
| 管理端与用户端混用 | 两套 API 须分路径；用户 token 不能调 admin 接口 |
| 缓存跳过鉴权 | Redis 命中后直接返回，未校验当前用户归属 |

**新接口 checklist**：列表 `WHERE user_id = 会话用户`；详情/改/删 load 后比对 `entity.userId`；不信 body/query 的 `userId`。

**基座内对照**：登录与 RBAC 见 `references/bases/{base-id}/`（如 ruoyi `SecurityUtils.getUserId()`、yisha `AuthorizeFilter`）。**禁止引用 skill 仓库外的项目路径。**
...
- 实现：`SET NX` + 正确 TTL；**禁止**硬编码超大 TTL 再 `expire` 覆盖

**TTL 样例**：B 2～10s · C 1h · D 回调 8h · D 不可逆标记 2～7d

Step C：每个写接口标注 A/B/C/D 与 TTL 依据。

**反例**：同一套回调去重逻辑 copy 到多个 Service — 应收口一个 dedup 组件 + D 类锁 + DB UNIQUE。
...
| 短信/验证码 | ~30min | 风控路径 Redis 不可用 → **503** |
| 日发送次数 / IP 限流 | 滑动窗口 | 可与 §3.1 B 配合 |
| 接口提交频率 | 1min 级 | 用户+接口维度计数 |
...
**默认不用 MQ**。普通 CRUD、单体内同步写库 + §3 Redis 锁/缓存即可；延迟任务优先 **DB job 表 + 基座调度**（如 ruoyi-quartz）。
...
| 文档 | 用途 |
|------|------|
| [references/bases/README.md](references/bases/README.md) | 基座内鉴权/Redis 落点对照 |
